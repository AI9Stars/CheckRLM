...
dpo_input_file = os.environ.get("DPO_INPUT_FILE", "")

def preprocessing(example, args, tokenizer):
    prompt = example["prompt"]
    if "qwen" in model_args.model_name_or_path.lower():
        if "Question:" in prompt:
            system_prompt = prompt.split("Question:")[0]
            user_prompt = "Question:" + prompt.split("Question:")[-1]
            template = "<|im_start|>system\n{system_prompt}<|im_end|>\n<|im_start|>user\n{user_prompt}<|im_end|>\n<|im_start|>assistant\n"
        else:
            system_prompt = prompt.split("Retrieved documents:")[0]
            user_prompt = "Retrieved documents:" + prompt.split("Retrieved documents:")[-1]
            template = "<|im_start|>system\n{system_prompt}<|im_end|>\n<|im_start|>user\n{user_prompt}<|im_end|>\n<|im_start|>assistant\n"
    ...
    prompt = template.format(system_prompt=system_prompt, user_prompt=user_prompt)
...
def main(script_args, training_args, model_args):
    ...
    if peft_config is None:
        # print("load origin model as ref_model!")
    ...
    dataset = load_dataset(script_args.dataset_name, name=script_args.dataset_config, data_files=dpo_input_file)
