...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run check_think_online with configurable parameters.")
...
def check_single_step(
    ...
    **model_kwargs
    ...
    if "</think>" in check_response:
        check_response = check_response.split("</think>\n\n")[-1]
    ...
    if subqueries == []:
        dataset_object["think_history"] += think.strip("\n") + "\n\n"
    ...
    dataset_object["think_history"] += refine_response.strip("\n") + "\n\n"
...
def process_single_sample(
    ...
    dataset_object["steps"] = current_step
    
    if "</think>" not in dataset_object["think_history"]:
        dataset_object["think_history"] = dataset_object["think_history"].rstrip("\n") + "\n</think>"
...
def check_think_online(
    ...
    **model_kwargs,
    ...
    if not resume_path:
        ...
        output_dir = os.path.join(project_root, output_root, dataset_name, 'refine_think', "online", f"{reasoning_model_name}-{check_model_name}", retrieval_method)
